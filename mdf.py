...
def mdf_b(S, drGs, constraints, x_max_default=0.01, x_min_default=0.000001, T=298.15, R=8.31e-3):
    ...
    for rxn in rxn_order:
        d = np.append(d, -drGs[drGs.rxn_id == rxn].drG.to_numpy()[0])
    # Transform drG values to units of RT
    d = d / (R*T)
    # Add x_max values
    for cpd in cpd_order:
        x_max = constraints[constraints.cpd_id == cpd].x_max.to_numpy()
        if len(x_max) == 0:
            # Use default x_max
    ...
    # Add x_min values
    for cpd in cpd_order:
        x_min = constraints[constraints.cpd_id == cpd].x_min.to_numpy()
        if len(x_min) == 0:
            # Use default x_max
...
def ratio_range(row):
    ...
    if row['spacing'] == 'lin':
        return np.linspace(row['ratio'], row['ratio_upper'],
                           int(row['ratio_step']))
...
def con_range(row):
    ...
    if row['spacing'] == 'lin':
        return np.linspace(row['x_min'], row['x_max'], int(row['steps']))
