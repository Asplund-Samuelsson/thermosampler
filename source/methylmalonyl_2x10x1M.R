...
sampling = bind_rows(
  foreach(
    f=list.files("results/methylmalonyl_2x10x1M", full.names=T)
  ) %dopar% {
...
S = read_tsv("data/methylmalonyl.stoich.tab") %>%
...
conc_ranges = read_tsv(
  "data/methylmalonyl.concentrations.tab",
...
ggsave("results/methylmalonyl.sampling_pca.2x10x1M.png", w=7, h=27, dpi=250)
...
ggsave("results/methylmalonyl.sampling_pca_compact.2x10x1M.png", w=7, h=4, dpi=250)
...
ggsave(
  "results/methylmalonyl.sampling_concs.2x10x1M.pdf", gp, w=210/25.4, h=600/25.4
)
...
ggsave(
  "results/methylmalonyl.sampling_concs_combo.2x10x1M.pdf",
  gp, w=210/25.4, h=210/25.4
)
...
ggsave(
  "results/methylmalonyl.sampling_dfs.2x10x1M.pdf", gp, w=210/25.4, h=400/25.4
)
...
ggsave(
  "results/methylmalonyl.sampling_dfs_combo.2x10x1M.pdf",
  gp, w=210/25.4, h=140/25.4
)
